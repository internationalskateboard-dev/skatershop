 ...
 * - Si el producto tiene solo una talla → añade directo al carrito.
 * - Si tiene varias tallas → redirige al detalle del producto.
 */
...
export default function ProductCard({ product }: { product: any }) {
  ...
  const handleAdd = () => {
    if (hasSizes) {
      router.push(`/products/${product.id}`); // 🔁 Ir al detalle para elegir talla
    } else {
      // 🔹 Si solo tiene una talla o ninguna, añadir directamente
      addToCart({
        id: product.id,
        name: product.name,
        price: product.price,
