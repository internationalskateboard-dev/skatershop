// components/admin/products/wizard/steps/ProductBaseStep.tsx
"use client";
...
export function ProductBaseStep({
  ...
  const [slugTouched, setSlugTouched] = useState<boolean>(Boolean(value.slug));
  ...
  const groups = Object.entries(groupMap).filter(([, ids]) => ids.length > 0);
    ...
    <section className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 space-y-4">
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-neutral-500 mt-1">
            Se guardará directamente en la tabla{" "}
      ...
      <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2">
        ...
        <div className="md:col-span-2 flex justify-end">
          ...
          >
            {isBusy ? "Guardando..." : "Guardar y continuar"}
          </button>
        </div>
      </form>
    </section>
